...
class MeanPoolCompress(Module):
    """
    Simple mean pooling over the tokens inside each compression window.
    Shape:
      input:  kv - [b, h, w, n, d]
      output: [b, h, w, d]
    """
    ...
    def forward(
        ...
        # average over the window-length dimension n
        return kv.mean(dim = -2)
