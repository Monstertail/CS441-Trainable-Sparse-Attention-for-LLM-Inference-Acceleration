...
def parse_args():
    ...
    parser.add_argument(
        '--loss_mode',
        type=str,
        choices=['kl_logits', 'kl_output_only', 'mse_hidden', 'mixed', 'ce'],
        default='kl_output_only',
        help=(
            'Loss mode: '
            'kl_output_only (default, KL only on output tokens), '
            'kl_logits (KL on all tokens), '
            'mse_hidden (MSE on hidden layers), '
            'mixed (KL+MSE), '
            'ce (pure cross-entropy, no distillation)'
        )
    )
    parser.add_argument(
        '--temperature',
